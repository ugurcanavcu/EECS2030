...
public class LZWCompressor {
	...
	private LZWDictionary sequence;
	...
	public LZWCompressor(String input) {
		...
		if (!input.isEmpty()) {
			...
			this.sequence = new LZWDictionary(input);
			this.input = input;
	...
	public List<Integer> encode() {
		...
		while (i < input.length()) {
			...
			String wc = w + c;
			if (this.sequence.contains(wc)) {

				w = wc;

			} else {

				int code = this.sequence.indexOf(w);
				result.add(code);
				this.sequence.add(wc);
				w = c;
		...
		result.add(this.sequence.indexOf(w));
		this.sequence.reset();
		return result;
	...
	public String decode(List<Integer> encoded) {
		...
		if (!encoded.isEmpty()) {
			...
			while (i < encoded.size()) {
				...
				if (this.sequence.hasIndex(next)) {

					l = this.sequence.get(next);
				} else {
				...
				result += l;
				String entry = this.sequence.get(prev) + l.substring(0, 1);
	...
	public double compressionRatio() {
		...
		double cr;
		cr = (double) this.input.length() / (double) this.encode().size();
		return cr;
